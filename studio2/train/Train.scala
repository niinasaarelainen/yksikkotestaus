...
class Train(val description: String) {
  ...
  /** Returns the car indicated by the given number, wrapped in an `Option`.
    * Returns `None` if no such car exists.
    *
    * @param numberOfCar  the number of the car that should be returned (>=1)
    */
  def car(numberOfCar: Int) = 
    if (numberOfCar < 1 && numberOfCar > this.numberOfCars) None else this.cars.lift(numberOfCar - 1)
